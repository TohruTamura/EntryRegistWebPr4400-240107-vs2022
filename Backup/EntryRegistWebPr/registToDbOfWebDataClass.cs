...
namespace EntryRegistWebPr {
	class registToDbOfWebDataClass {
		public static string RegistToDbOfWebEntryList() {
			...
			if (webList != null) {
				for (int j = 0; j < webList.Count; j++) {
					if (!isRegisted(webList[j])) {
						if (doRegist(j,webList[j], ref mes)) {
							//mailTableにデータを保存する
							mes+=registToDbOfMailAddress(webList[j]);
							setFalseOfEachWebEntryFlag(webList[j]);
							count++;
						}
					}
					else{
						setFalseOfEachWebEntryFlag(webList[j]);
						mes += getRegistedErrorMessage(j, webList[j]);
					}
				}
			}
			else{
		...
		private static string getMailAddressInsertSql(WebEntryClass aWebEntryC) {
			...
			return string.Format(sql, etcClass.objToInt(aWebEntryC.jcbl[0]), aWebEntryC.name[0], aWebEntryC.mailAddress);
		}
		
		private static string getRegistedErrorMessage(int pos,WebEntryClass aWebEntryC){
			return string.Format("{0}: id={1} {2} {3} {4} {5}は既に受付データベースに登録されています。"+Environment.NewLine,
										(pos + 1).ToString(),
		...
		private static bool isRegisted(WebEntryClass aClass) {
			for (int j = 0; j < aClass.jcbl.Count; j++) {
				if (aClass.jcbl[j]!="0"){
					string sql = string.Format("SELECT COUNT(*) FROM Team WHERE Event={0} AND ( (Team.[1Number]={2} AND Team.[1Name]='{1}') OR (Team.[2Number]={2} AND Team.[2Name]='{1}') OR  (Team.[3Number]={2} AND Team.[3Name]='{1}') OR (Team.[4Number]={2} AND Team.[4Name]='{1}') OR (Team.[5Number]={2} AND Team.[5Name]='{1}') OR (Team.[6Number]={2} AND Team.[6Name]='{1}') ) ", aClass.id, aClass.name[j], aClass.jcbl[j]);
		...
		//登録された場合はtrueを返す
		private static bool doRegist(int pos,WebEntryClass aWebC, ref string mes) {  //一部改変
			string sql = getInsertSqlOfEntryTableByWebEntry(aWebC);
			string errorMes = MicrosoftAccessClass.executeSQL(sql, EntryMdbClass.getIMP_SumConnectStr(new entryFileClass().FileFullPath));
			if (errorMes == "") {
				mes += getWevEntryMessage(pos,aWebC) + Environment.NewLine;
				return true;
			}
			else {
				mes += errorMes + Environment.NewLine;
				return false;
			}
		}

		private static string getWevEntryMessage(int pos, WebEntryClass aWebEntryC) {
			return string.Format("{0}: id={1} {2} {3} {4} {5} {6}を登録",
									(pos + 1).ToString(),
									aWebEntryC.id.ToString(),
									aWebEntryC.eventDate,
									aWebEntryC.eventName,
