...
namespace EntryRegistWebPr {
	class newFtpServerClass {
		...
		public string getWebTempFolder(){
			...
			// on 22/10/06 ftpDefaultFolderが空白の場合の処理を追加。
			// on 22/10/07 fptServerNameの後 /に,ftpDefaultFolderが空の場合
			if (this.ftpDefaultFolder == "") {　//for sakura vps-2
				return string.Format("{0}/{1}/temp/", this.fptServerName, this.entryFolderName);
			}
			else {
				return string.Format("{0}//{1}/{2}/temp/", this.fptServerName, this.ftpDefaultFolder, this.entryFolderName);
			}
		}
